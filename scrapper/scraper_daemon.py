...
class IdFromUrlTest(unittest.TestCase):
    def testIdFromUrl(self):
        self.assertEqual(1000124, id_from_url("https://dilbert.com/strip/2000-01-24"))
...
class TestTrimWhiteSpaces(unittest.TestCase):
    ...
    def testSeveralWords(self):
        self.assertEqual(
            "several words to test",
            trim_whitespaces("\n  several  \twords \n\n to test"),
        )


def extract_text(img_path):
    return trim_whitespaces(to_ascii(pytesseract.image_to_string(Image.open(img_path))))
...
def main():
    try:
        conn = mdb.connect(
            host="localhost", user="dorian", password="moijemon", database="geeky"
        )
        cursor = conn.cursor()
